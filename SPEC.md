...
10. Generate Movement Log / Confirm Mapping output.
...
- Do not determine Virtual merely by worksheet ordinal (e.g. 'second sheet'). Use configured/recognized sheet names.
...
- Upcoming Phase Out is display/annotation only and must **not generate Movement Log or status-change log entries**.
...
## 7. Rename / Merge — mandatory batch evaluation
...
- Formal Movement Log is generated when Confirm Mapping compares final Converted Data to Online Baseline.

## 10. Mapping evidence / recommendation (POC)
...
Also provide Mapping/Movement Log output (sheet or separate export according to POC implementation).
